...
export function setMemberDetail(memberDetail, useCookie = true) {
  return dispatch => {
    return new Promise((resolve, reject) => {
      try {
        if (useCookie) {
          cookie.setCookie(globalKeys.ACCESS_MEMBER_DETAIL, JSON.stringify(memberDetail));
        } else {
          sessionStorage.setItem(globalKeys.ACCESS_MEMBER_DETAIL, JSON.stringify(memberDetail));
        }
...
export function getMemberDetail() {
  return dispatch => {
    return new Promise((resolve, reject) => {
      try {
        let savedDetail = sessionStorage.getItem(globalKeys.ACCESS_MEMBER_DETAIL);
